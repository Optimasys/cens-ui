/**
 * POST /api/submit-iecregis
 *
 * Handles IEC competition form submissions.
...
export async function POST(request: NextRequest) {
  try {
    ...
    const sheetsWebhookUrl = process.env.GOOGLE_SHEETS_WEBHOOK_URL_IECREGISTER;
    if (sheetsWebhookUrl) {
      const sheetsData = formatDataForSheets('iec-regis', {
        teamName: validData.teamName,
        competitionType: validData.competitionType,
        teamLeader: validData.teamLeader,
        student2: validData.student2,
    ...
    console.error('Submit IEC competition error:', message);
